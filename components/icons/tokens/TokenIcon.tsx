...
import { checkIsUnknownToken } from "@/lib/utils/tokenTags";
...
function genImageTransformURL(url: string): string {
...
const UnknownTokenImage: React.FC<{
  ...
  return (
    <svg
      ...
      data-url={imageUrl}
      data-transformed-url={genImageTransformURL(imageUrl || '')}
    >
      <path
...
const TokenIcon: React.FC<TokenIconProps> = ({
  ...
  const ImageComp = React.useMemo(() => {
    // If both transformerHasError and hasError, render fallback
    ...
    // Try transformed image via wsrv.nl
    return (
      <img
